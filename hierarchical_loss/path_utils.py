...
def filter_empty_paths(predicted_paths: list[list[int]], return_indices: bool = False) -> list[list[int]] | tuple[list[list[int]], list[int]]:
    """Filters out empty paths from a list of predicted paths.

    This function removes empty paths. Optionally, it returns the indices
    of the kept paths, allowing the caller to filter parallel data structures
    like bounding boxes or scores.
    ...
    return_indices : bool, optional
        Whether to return the indices of the kept paths, by default False.

    Returns
    -------
    list[list[int]] | tuple[list[list[int]], list[int]]
        The filtered list of paths. If `return_indices` is True, returns
        a tuple containing the filtered paths and a list of kept indices.

    Examples
    --------
    >>> paths = [[4], [4, 6], [4, 5], [], []]
    >>> filter_empty_paths(paths)
    [[4], [4, 6], [4, 5]]
    >>> f_paths, keep_idx = filter_empty_paths(paths, return_indices=True)
    >>> keep_idx
    [0, 1, 2]
    """
    keep_idx = [i for i, path in enumerate(predicted_paths) if len(path) > 0]
    f_paths = [predicted_paths[k] for k in keep_idx]
    if return_indices:
        return f_paths, keep_idx
    return f_paths


def batch_filter_empty_paths(predicted_paths: list[list[list[int]]], return_indices: bool = False) -> list[list[list[int]]] | list[tuple[list[list[int]], list[int]]]:
    """Applies empty path filtering to a batch of predicted paths.
    ...
    return_indices : bool, optional
        Whether to return the indices of the kept paths for each batch item, by default False.

    Returns
    -------
    list[list[list[int]]] | list[tuple[list[list[int]], list[int]]]
        A list containing the filtered paths for each item in the batch.
        If `return_indices` is True, each item is a tuple of (filtered_paths, keep_indices).

    Examples
    --------
    >>> paths_batch = [[[4], [4, 6], [4, 5], [], []], [[4], [], [4, 5]]]
    >>> result = batch_filter_empty_paths(paths_batch, return_indices=True)
    >>> result[0][1] # indices for first batch item
    [0, 1, 2]
    >>> result[1][1] # indices for second batch item
    [0, 2]
    """
    return [filter_empty_paths(paths, return_indices=return_indices) for paths in predicted_paths]
